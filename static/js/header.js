(() => {
  ...
  var _ticker = (function () {
    ...
    _self = {
      ...
      remove: function remove(callback, i) {
        ~(i = _listeners3.indexOf(callback)) &&
          _listeners3.splice(i, 1) &&
          _i2 >= i &&
          _i2--;
      },
  ...
  var _getTarget = function _getTarget2(t, self) {
    return (
      ...
      (typeof t === "string" && gsap2.config().nullTargetWarn !== false
  ...
  var _maxScroll = function _maxScroll2(element, _ref2) {
    ...
    return Math.max(
      ...
      (s = "scroll" + d2) && (a = _getProxyProp(element, s))
        ...
        : _isViewport3(element)
          ? (_docEl2[s] || _body2[s]) - _getViewportDimension(d2)
  ...
  var elements = document.querySelectorAll(".opacity-0");
  elements.forEach((el) => {
    gsapWithCSS.to(el, {
      opacity: 1,
      y: 0,
  ...
  window.addEventListener("scroll", () => {
    ...
    if (
      ...
      gsapWithCSS.to(header, { y: -100, duration: 0.3, ease: "power1.out" });
      isHidden = true;
    } else if (currentScrollY < lastScrollY && isHidden) {
      gsapWithCSS.to(header, { y: 0, duration: 0.3, ease: "power1.out" });
      isHidden = false;
    }
